...
import fetchPosts from "../../../components/posting/PollingPosts";
...
export default function Sidebar({ isOpen, setIsOpen }) {
  ...
  return (
    <>
            ...
            {isAboutOpen && (
              <div className="mt-2 p-4 bg-[#1A2B44] rounded-md text-[#F7FAFC] overflow-y-auto max-h-[100px] custom-scrollbar">
                <p className=" text-sm font-semibold italic">
                  Droplet is an open forum for all accepting all valid opinions
                  and topics, a hub for communication, a stage to ripple
                  discussions
                </p>
              </div>
            )}
          ...
          <div className="text-sm flex justify-around text-gray-500">
            <Link to="../feedback_form">
              <button>
                ...
                <span className="text-blue-700 hover:text-blue-400">here</span>
              </button>
            </Link>
          </div>
        </div>
      ...
      {/* Toggle Button */}
      <button
        className={`h-screen top-0 left-0 bottom-0 w-[30px] fixed border-none text-[#feffff] text-[30px] cursor-pointer z-[1001] transition-all duration-500 ease-in-out focus:outline-none focus:shadow-none focus:border-none -translate-x-[7px] hover:cursor-pointer hover:bg-blue-400 ${
          isOpen ? "left-[210px] top-[0px]" : "left-[5px]"
        }`}
